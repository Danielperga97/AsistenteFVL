...
def generateMessage(emotion):
    ft=open("/media/pi/PACIENTE/frases/tmp.txt","w")
    with open("/media/pi/PACIENTE/frases/"+emotion+".txt","r") as fp:
        lines= fp.readlines()
        cnt=0
        phrase=lines[cnt]
        for line in lines:
    ...
    fp.close()
    ft.close()
    os.system("rm /media/pi/PACIENTE/frases/Feliz.txt")
    os.system("mv /media/pi/PACIENTE/frases/tmp.txt /media/pi/PACIENTE/frases/Feliz.txt")
    print(phrase)
